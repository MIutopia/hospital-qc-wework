...
| 阻塞项 | 影响任务 | 需要谁 | 预计解除 |
|--------|---------|--------|---------|
| 企微通讯录读取受「企业可信 IP」白名单限制（errcode=60020） | 任务 2.4 医生-企微映射（改为企微通讯录 API 直拉） | 企微管理员 / PM | 待后台加白部署服务器公网 IP 后复测；**前置：先添加应用「可信域名」**（2026-08-03 确认） |
...
- [ ] 防火墙白名单（PM 目标第3周末到位）
- [x] **企微应用「可信域名」+「企业可信IP」配置顺序确认**（2026-08-03：先添加可信域名并完成归属验证，才能配置可信 IP）
...
> **内网部署补充（2026-08-03）：** 院方服务器在内网（无公网 IP / NAT 出网），H5 需公网入口（端口映射 / 云服务器反代+FRP / 专线三选一）；企微「可信IP」前置必须先添加「可信域名」并完成归属验证；Nginx 配置与网络要求详见 [内网部署与企微配置方案](内网部署与企微配置方案.md)。
...
| 2026-08-03 | 技术负责人 | V2.6 更新：确认 GitHub Projects 平台限制 — 个人账户 + Fine-grained PAT 无法访问/管理 user-owned Projects（权限界面无「Projects」选项），API 无法自动创建 Project v2；替代方案：手动建 Board 导入 Issues，或直接用 Issues+标签+里程碑作为看板（详见 README） |
| 2026-08-03 | 技术负责人 | V2.7 更新：Project v2 看板创建成功 — 切换经典 token（含 project 权限）后经 GraphQL 创建「住院病例质控系统看板」（https://github.com/users/MIutopia/projects/1），13 个 Issue 全部挂入并分配中文状态列（待开始/进行中/阻塞/已完成）；README 看板章节同步更新 |
| 2026-08-03 | 技术负责人 | V2.8 更新：确认企微配置前置条件 — 企业微信应用「企业可信IP」添加的前提是**必须先添加「可信域名」并完成归属验证**（网页授权及JS-SDK）；已同步更新 PM 阻碍报告与 M4 前置依赖 |
| 2026-08-03 | 技术负责人 | V2.9 更新：新增《内网部署与企微配置方案》— 覆盖内网网络拓扑（公网入口三选一：端口映射/云服务器反代+FRP/专线）、企微配置顺序（备案→可信域名→可信IP→权限）、Nginx 分场景配置（含可信域名验证文件位置）、防火墙要求清单与实施步骤；同步更新 deploy/nginx.conf 注释与 M7 部署章节 |
| 2026-08-03 | 技术负责人 | V2.10 更新：院方现状补充 — 院方已有一台运行其他业务的 Nginx 服务器；《内网部署与企微配置方案》新增「院方现有 Nginx 复用评估」章节（情形1：有公网入口则优先复用新增 server 块；情形2：仅内网则仍需云服务器入口），并列出待信息科确认的复用决策清单 |
| 2026-08-03 | 技术负责人 | V2.11 更新：确认院方现有 Nginx 与质控后端**同一内网**（无公网入口）→ 结论：该 Nginx 不能作为公网入口，仅可作内网反代层；《内网部署与企微配置方案》明确两条链路（①云服务器 FRP 直连质控后端【推荐】；②云服务器→院内 Nginx→质控后端） |
| 2026-08-03 | 技术负责人 | V2.12 更新：**院方确认有公网 IP** → 部署方案调整：方案 A（公网端口映射）升为首选；公网入口链路重排为 ①公网IP→院内现有Nginx→质控后端（首选，复用现有Nginx新增server块）②公网IP→质控后端机Nginx（备选）③云服务器+FRP（备用）；《内网部署与企微配置方案》同步更新 |
| 2026-08-03 | 技术负责人 | V2.13 更新：院方确认**有备案域名**且 **Nginx 服务器可出外网**（疑似出口网关机）→ 首选链路①基本成立；新增「企微 API 出站路径」章节：需确认**质控后端能否出外网**（情形A直连【推荐】/ 情形B经Nginx服务器代理），企业可信IP按实际出站公网IP填写 |
| 2026-08-03 | 技术负责人 | V2.14 更新：**部署方案定稿** — 院方确认公网IP:443 映射到院内 Nginx（首选链路①）+ 质控后端放行出外网（情形A直连）；新增 `deploy/nginx-qc-server-block.conf`（院内 Nginx 新增 server 块模板：HTTPS+可信域名验证+API反代+H5静态+安全头）；企业可信IP=后端出站NAT公网IP；《内网部署与企微配置方案》标记最终确认结论 |
