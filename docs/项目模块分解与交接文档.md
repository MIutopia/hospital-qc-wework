...
3. ✅ **数据库连接验证** — 业务库 HospitalQC 与 HIS 仓库 med_record 连接均成功（2026-07-31）
...
| 日期 | 问题 | 原因 | 状态 | 解决方案 |
|------|------|------|------|---------|
| 2026-07-31 | 服务启动失败 `yaml: cannot unmarshal !!str \`1000008\` into int` | config.yaml 中 `agent_id: "1000008"` 为字符串，代码字段为 int | ✅ 已解决 | 去掉引号改为 `agent_id: 1000008` |
...
| 2026-07-31 | 建表后 `GET /admin/rules`、`/admin/doctors` 等分页接口返回 500「查询失败」 | go-mssqldb 对 `OFFSET ? ROWS FETCH NEXT ? ROWS ONLY` 与 `SELECT TOP (?)` 的 `?` 占位符支持不佳（报 `mssql: "?"附近有语法错误`）；普通查询正常 | ✅ 已解决 | DAO 层将分页 offset/pageSize 与 TOP limit 作为整数直接内联进 SQL（rule/case/push_log/doctor/his 共 5 个 DAO）。`go test ./...` 通过；实测 /admin/rules 返回 10 条规则、/admin/doctors 返回 5 位医生 |
...
**状态：** ⏳ 待开始（表结构已确认，连接参数已填写并验证通过 2026-07-31）
**负责人：** TBD
**前置依赖：**
- [ ] M1 基础设施完成
...
- [ ] 同步日志记录（待完善）
- [x] HIS 连接验证通过（2026-07-31，`med_record` 库连接 + Ping 成功）；全量同步联调待脱敏样本数据
...
| DSL operator | expr 实现 |
|-------------|-----------|
| IS_NULL | `target == nil` |
| IS_NOT_NULL | `target != nil` |
| EQUALS | `target == value` |
| NOT_EQUALS | `target != value` |
| GREATER_THAN | `target > threshold` |
| LESS_THAN | `target < threshold` |
| HOURS_SINCE | `hoursSince(reference, target) > threshold`（自定义函数） |
| CONTAINS | `contains(target, substring)` |
| REGEX | `matches(target, pattern)`（自定义函数） |
| CROSS_CHECK | 多字段组合表达式 |
...
#### 交付物
- [ ] 规则 DSL 类型定义 + Schema 校验
- [ ] expr-lang/expr 解析引擎集成
- [ ] 质控执行器（并发批次执行）
- [ ] 规则 CRUD 管理接口
- [ ] 单元测试（至少覆盖 8 种运算符）
...
| 2026-07-31 | 技术负责人 | V1.6 更新：数据库与企微连通性验证通过 — 业务库 HospitalQC + HIS 仓库 med_record 连接成功，企微 access_token 获取成功（curl 直测 + 服务内 TokenManager 刷新）。修复：`agent_id` 配置类型（字符串→整数）；旧 exe 的 `encrypt=false` 触发 TLS 1.0 握手失败，源码 DSN 已用 `encrypt=disable` 解决并重新编译；企微 secret 需经 `WEWORK_AGENT_SECRET` 环境变量注入（config.yaml 的 agent_secret 字段被 `yaml:"-"` 忽略）|
...
| 2026-07-31 | 技术负责人 | V1.8 更新：修复 go-mssqldb 分页/TOP 占位符 bug（`OFFSET/FETCH` 与 `TOP (?)` 的 `?` 参数导致分页接口 500），DAO 层内联整数；建表后实测 /admin/rules(10)、/admin/doctors(5) 返回正常；HIS 同步查询在 med_record 复现成功（caseman/admrec TOP5 各 5 行），M2 同步链路就绪 |
