...
> **院方现状（2026-08-03 补充）：** 院方已有一台部署 Nginx 的服务器，该服务器**同时运行其他业务**，且**与质控后端同处统一内网**（无公网入口）。
> 结论：该 Nginx **不能作为公网入口**，只能作为可选的**内网反代层**；公网入口仍需云服务器或端口映射（见 [二、院方现有 Nginx 复用评估](#二院方现有-nginx-复用评估)）。
...
### 当前现状：该 Nginx 仅内网使用（同一内网，无公网入口）→ **不能作为公网入口**

- 公网入口仍按「公网入口三选一」实施（推荐**方案 B：云服务器反代 + FRP/端口映射**）
- 院方现有内网 Nginx 可选作为**内网反代层**（两种链路二选一）：

**链路 ①（推荐，简单）：云服务器 → FRP 隧道 → 质控后端 :8080（跳过院内 Nginx）**

```
医生手机 ──HTTPS──> 云服务器 Nginx（公网入口）
                     ├─ /          → H5 静态
                     ├─ /api/*     → FRP 隧道 ──> 质控后端 :8080（内网）
                     └─ /WW_verify_xxx.txt → 可信域名验证文件
```

**链路 ②（院方要求走院内 Nginx 时）：云服务器 → FRP 隧道 → 院内 Nginx（内网反代）→ 质控后端 :8080**

```
医生手机 ──HTTPS──> 云服务器 Nginx ──> FRP 隧道 ──> 院内 Nginx（同一内网，反代）──> 质控后端 :8080
```

- 链路 ② 的前提：院内 Nginx 与质控后端**同内网可直连**（已确认成立），信息科在该 Nginx 上新增一个内网 server 块（`proxy_pass http://质控后端IP:8080`）即可，不影响其他业务
- 协调要点（若走链路 ②）：
  - 备份 nginx.conf → 新增内网 server 块 → `nginx -t` → reload → 验证其他业务不受影响
  - 该 Nginx 仅做内网转发，**不需要**证书/域名（TLS 在云服务器终结）

### 复用决策清单（待院方信息科确认）

- [x] 该 Nginx 服务器**无公网入口**（同一内网）—— 已确认
- [ ] 是否允许在院内 Nginx 上**新增内网 server 配置块**（转发到质控后端，不影响其他业务）？
- [ ] 公网入口方式确认：云服务器（推荐）/ 医院公网 IP 端口映射
- [ ] 若走链路 ②，确认质控后端内网 IP 与端口（8080）
