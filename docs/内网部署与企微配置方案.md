...
> ② **院方具备公网 IP**；
> ③ **有备案域名**（可作可信域名 / H5 域名）；
> ④ **Nginx 服务器可出外网**（具备互联网访问能力，疑似医院互联网出口网关机）。
> 结论：**首选链路①（公网IP:443 → 院内Nginx → 质控后端）基本成立**。
> ⚠️ **待确认关键点：质控后端服务器能否出外网？**——企微 API（gettoken/message/send）由后端直接调用，若后端无法出网需经 Nginx 服务器代理或信息科放行（见 [四、企微 API 出站路径](#四企微-api-出站路径)）。
...
- [ ] **公网 IP 的 443 可映射到哪台内网机器**：院内现有 Nginx 服务器 / 质控后端机？
- [ ] 是否允许在院内 Nginx 上**新增 server 配置块**（不影响其他业务）？
- [ ] 公网 IP 是否为**固定 IP**（出站 NAT 与入站是否同一 IP，供企业可信IP使用）？
- [ ] **质控后端服务器能否出外网？**（决定企微 API 直连或经 Nginx 服务器代理，见第四章）
...
| 情形 | 说明 | 企业可信IP 填什么 |
|------|------|------------------|
| A. 质控后端可直接出外网 | 信息科给后端服务器放行出站 443 到 `qyapi.weixin.qq.com` | 后端出站 NAT 公网 IP（若与入站同一公网 IP 则最简） |
| B. 质控后端不可出网，经 Nginx 服务器代理 | 后端通过正向代理（HTTPS_PROXY）走 Nginx 服务器出网；或信息科在防火墙上把后端出站 NAT 到出口网关 | Nginx 服务器出口公网 IP |

- **推荐**：优先争取情形 A（信息科放行后端出站 443），后端零改动、链路最短
