...
1. **数据库连接验证** — 用院方实际 SQL Server 2014 信息验证连接池
2. **建表脚本执行** — DBA 在目标数据库运行 sql/ 目录下的 3 个脚本
3. **企业微信凭证验证** — 配置 CorpID/Secret 后验证 access_token 获取
4. **本地全量编译测试** — `go build ./...` 确保无编译错误
5. **第1周任务卡中其余任务** — 详见 [第1周开发任务卡.md](docs/第1周开发任务卡.md)
...
**状态：** ⏳ 待开始（表结构已确认，待信息科填写连接参数）
**负责人：** TBD
**前置依赖：**
- [ ] M1 基础设施完成
- [x] HIS 表结构文档（已从 HIS 数据仓库确认）
- [ ] HIS 只读账号连接参数（IP/密码待信息科填写）
- [ ] 若 HIS 连接参数未到：先用 CSV 手工导入走阶段一

#### 技术方案要点（2026-07-31 更新：基于 HIS 数据仓库确认）

> **重大更新：** HIS 数据仓库（`med_record` 库）表结构已确认，质控系统**直接读取数据仓库宽表**，不直连 SBO 源库。部署与数据库同服务器。

1. **数据源（已确认）**
   - 住院数据仓库：`med_record` 库（**不使用财务库 `caiwu`**）
   - 病案首页：`hospitalisation_case_man`（含入院/出院时间、科室、医生、诊断、手术、病历质量）
   - 入院记录：`med_record_hospitail_rceord`（主诉、现病史、体格检查）
...
#### 交付物
- [x] HIS 数据同步模块（增量拉取，读 `med_record` 宽表）— 2026-07-31 完成
...
```mermaid
graph TD
    subgraph 单机 "WIN-OKT2FAKMULV（SQL Server 2014 服务器，应用同机部署）"
...
> **部署说明（2026-07-31 更新）：**
> - 后端、Nginx、HIS 数据仓库、HospitalQC 数据库**全部在同一台服务器** `WIN-OKT2FAKMULV\SQLEXPRESS`
...
| 依赖 | 用途 | 状态 | 责任人 |
|------|------|------|--------|
| HIS 数据仓库表结构 | 数据同步 | ✅ 已确认（`med_record`/`records`/`caiwu` 三库宽表） | 信息科 |
| HIS 数据库连接参数（IP/密码） | 数据同步 | ⏳ 待信息科填写（见连接信息确认表） | 信息科 |
| 企业微信 CorpID/Secret | 消息推送 | ✅ 第1天到位 | PM |
| 域名备案 + HTTPS | H5 页面 | ⏳ 备案中（1-2周） | PM |
| 防火墙白名单 | 出站到企微 API | ⏳ 第3周前 | PM |
| Windows Server（与数据库同机） | 部署 | ✅ 确认单机部署（`WIN-OKT2FAKMULV`） | 院方 |
...
| 里程碑 | 计划日期 | 状态 | 实际完成 | 备注 |
|--------|---------|------|---------|------|
| M1 基础设施完成 | 第1周末 | 🔷 脚手架代码完成，等待团队认领 | - | 28 个 Go 文件已预搭建 |
...
|------|---------|------|------|------|---------|--------|------|
| R01 | HIS 表结构不明，数据同步阻塞 | 低 | 高 | 🟡 | ✅ 表结构已确认（HIS 数据仓库宽表），待信息科填 IP/密码 | 信息科 | ✅ 大部分解除 |
...
| 2026-07-31 | 技术负责人 | V1.3 更新：确认 HIS 数据仓库表结构（med_record 宽表），M2 改读数据仓库；部署改为单机（应用与数据库同服务器）|
